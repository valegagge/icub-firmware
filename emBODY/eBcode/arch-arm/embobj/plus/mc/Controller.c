...
static void get_jomo_coupling_info(const eOmc_4jomo_coupling_t *jomoCouplingInfo, EOconstarray* carray)
{
    ...
    for(int s=0; s<o->nSets; s++)
    {
        ...
        //o->jointSet[s].veldir_ctrl_out_type = jsetcfg[s].pid_output_types.veldir_ctrl_out_type; VALE X VEL DIR
        o->jointSet[s].veldir_ctrl_out_type = eomc_ctrl_out_type_vel;
        o->jointSet[s].posdir_ctrl_out_type = eomc_ctrl_out_type_vel;
